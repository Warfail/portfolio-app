...
// Add experience image upload handler
    ...
    try {
        ...
        if (response.ok) {
            ...
            document.getElementById('exp-image-preview').innerHTML = `
                <img src="${imageUrl}" alt="Company Logo" style="max-width:200px;border-radius:8px;">
            `;
            
            // Update the experience with the image URL
...
// Update loadExperiences to show images
        ...
        container.innerHTML = experiences.map(exp => `
            <div class="admin-item" data-id="${exp.id}">
                <div class="admin-item-content" style="display:flex;align-items:center;gap:16px;">
                    ${exp.image_url ? `<img src="${exp.image_url}" style="width:60px;height:60px;object-fit:cover;border-radius:8px;" />` : ''}
                    <div>
...
// Update editExperience to show image
async function editExperience(id) {
    try {
        ...
        if (exp.image_url) {
            document.getElementById('exp-image-preview').innerHTML = `
                <img src="${exp.image_url}" alt="Company Logo" style="max-width:200px;border-radius:8px;">
            `;
        }
...
document.getElementById('skill-cancel-btn').addEventListener('click', cancelSkillEdit);
...
async function loadExperiences() {
    try {
        ...
        container.innerHTML = experiences.map(exp => `
            <div class="admin-item" data-id="${exp.id}">
                <div class="admin-item-content" style="display:flex;align-items:center;gap:16px;">
                    ${exp.image_url ? `<img src="${exp.image_url}" style="width:50px;height:50px;object-fit:cover;border-radius:8px;" />` : ''}
                    <div>
                        <h4>${exp.position} at ${exp.company}</h4>
                        <div class="meta">${new Date(exp.start_date).toLocaleDateString()} - ${exp.is_current ? 'Present' : new Date(exp.end_date).toLocaleDateString()}</div>
                    </div>
...
async function editExperience(id) {
    try {
        ...
        if (exp.image_url) {
            document.getElementById('exp-image-preview').innerHTML = `
                <img src="${exp.image_url}" alt="Company Logo" style="max-width:150px;border-radius:8px;">
